...
public class PluginResearchCatalog {
    ...
    public PluginResearchCatalog(PluginConnectorRegistry registry, NativeServiceGatewayRouter services) {
        integrations = List.of(
            entry(registry, services, "P01", "0510闪电三件套.zip", "闪电三件套", "运动计划", "PLAINTEXT", "NATIVE_PARTIAL", "flash", null,
            ...
            entry(registry, services, "P02", "benzs.zip", "Benz 同步桥", "课程同步", "PLAINTEXT", "EXISTING", "27", null,
            ...
            entry(registry, services, "P03", "heisha_sdxy_user.zip", "黑鲨闪动校园", "运动计划", "PLAINTEXT", "NATIVE_PARTIAL", "heisha", null,
            ...
            entry(registry, services, "P04", "jiguang_user.zip", "极光", "运动计划", "PLAINTEXT", "NATIVE_PARTIAL", "jiguang", null,
            ...
            entry(registry, services, "P05", "ssbenz.zip", "永夜 / ssbenz", "运动计划", "MIXED", "NATIVE_PARTIAL", "ssbenz_xbd", null,
            ...
            entry(registry, services, "P06", "sxdk_tw.zip", "实习计划套娃", "实习计划", "PLAINTEXT", "NATIVE_PARTIAL", "sxdk_tw", null,
            ...
            entry(registry, services, "P07", "syyv5 (1).tar.gz", "syyv5 多项目中心", "项目与账务", "PLAINTEXT", "NATIVE_PARTIAL", "syyv5", null,
            ...
            entry(registry, services, "P08", "toc模板专属对接鲸鱼文件.zip", "鲸鱼 · TOC 版", "运动计划", "MIXED", "NATIVE_PARTIAL", "jingyu", null,
                List.of("Keep、步道乐跑与校园运动报价、账号查询及下单", "学校查询、跑步规则与最低距离校验", "逐次任务安排、延期与暂停恢复", "双编号及学校身份核对、人工退款入账"),
                List.of("三个项目的 Java/Vue 业务流程已实现；校园运动尚未部署和真实联调", "体育项目的图形验证、规则更新、补跑计费仍待补齐；设备授权和推送没有独立可核实接口", "退款没有原子数量回执，必须核对实际数量；同名学校无法明确区分时不能下单或关联；不执行 PHP 或定时脚本，不重发结果不确定的订单"),
                List.of("jingyu/api.php（静态核对的固定表单协议）", "index/keep.php", "index/bdlp.php", "jingyu/jingyu_tables.sql")),
            entry(registry, services, "P09", "二开appui对接.zip", "AppUI 实习打卡", "实习计划", "MIXED", "NATIVE_PARTIAL", "appui", null,
            ...
            entry(registry, services, "P10", "无心闪动0312.zip", "无心闪动", "运动计划", "PLAINTEXT", "NATIVE_PARTIAL", "wuxin", null,
            ...
            entry(registry, services, "P11", "通用鲸鱼套娃对接.zip", "鲸鱼 · 通用版", "运动计划", "DUPLICATE", "DUPLICATE", null, "P08",
            ...
            entry(registry, services, "P12", "雷电_v1.2_二开对接.zip", "雷电 v1.2", "运动计划", "MIXED", "NATIVE_PARTIAL", "leidian", null,
    ...
    private static PluginIntegrationDescriptor entry(PluginConnectorRegistry registry, NativeServiceGatewayRouter services,
            String id, String archive,
        ...
        // Implemented actions are not activation/availability claims. Runtime gates still apply.
        List<String> serviceCapabilities = "NATIVE_PARTIAL".equals(status)
                ? services.capabilities(type) : List.of();
        return new PluginIntegrationDescriptor(id, archive, name, category, evidenceLevel, status, type, duplicateOf,
                observed, blockers, evidence, available, serviceCapabilities,
                connector == null ? List.of() : connector.projects());
    }
}
