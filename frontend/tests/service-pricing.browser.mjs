...
const names = ['exact', 'legacy', 'refund', 'admin', 'daily', 'free', 'invalid', 'long'];
...
async function open(){administrative.value=kind.value==='admin';quote.value=await(administrative.value?getAdminServiceOperation:getServiceOperation)(ids[kind.value]);}
...
Object.assign(quotes.get(ids.refund), {action:'REFUND',quantity:6,amount:'0.32',amountLabel:'预计退款上限'});
Object.assign(quotes.get(ids.admin), {action:'SETTLE_REFUND',quantity:6,amount:'0.32',amountLabel:'预计退款上限'});
...
try {
  ...
  await page.route('**/*',async route=>{
    ...
    if(method==='POST'&&match[3]){
      ...
      if(quote.id===ids.exact)return route.abort('connectionreset');
      return respond();
    }
  ...
  console.log('PASS exact service pricing: real Vue quote details, eight-decimal strings, legacy prices, capped admin refund, service-day units, invalid/free suppression, one lost confirmation and original GET recovery, 44px mobile dark/long values, no business storage; mocked APIs only');
