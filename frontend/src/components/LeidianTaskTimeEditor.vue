<template>
  <el-dialog v-model="open" title="修改任务时间" class="leidian-task-editor" width="min(560px, 95vw)"
...
import { leidianClock, leidianDateRange, leidianTaskTimeError } from "@/utils/leidianServices";
...
function taskValid() {
  return typeof props.task?.id === "string" && /^[A-Za-z0-9_-]{1,64}$/.test(props.task.id) && !/\p{Cc}/u.test(props.task.id) &&
    Number.isInteger(props.task.page) && props.task.page >= 1 && props.task.page <= 100 && typeof props.task.orderId === "string";
}
const validation = computed(() => !taskValid() ? "请选择有效的任务记录" : leidianTaskTimeError(date.value, time.value));
...
async function preview() {
  if (busy.value || !props.modelValue || !taskValid() || !accessToken.value || leidianTaskTimeError(date.value, time.value)) return;
  ...
  try {
    const quote = await previewServiceAction(task.orderId, { action: "CHANGE_TIME", quantity: 0,
      fields: { taskId: task.id, page: String(task.page), time: `${date.value} ${leidianClock(time.value)}` } });
...
:global(.leidian-task-editor) { display: flex; flex-direction: column; max-height: calc(100dvh - 48px); margin: 24px auto !important; }
:global(.leidian-task-editor .el-dialog__body) { overflow: auto; }
:global(.leidian-task-editor .el-input__wrapper), :global(.leidian-task-editor .el-button) { min-height: 44px; box-sizing: border-box; }
</style>
